...
const transform: AxiosTransform = {
  ...
  transformResponseHook: (res: AxiosResponse<Result>, options: RequestOptions) => {
    ...
    // 兼容 message 和 msg 两种字段名
    const { code, msg } = res.data;
    const msgText = msg || (res.data as any).message;
    ...
    switch (code) {
      ...
      case ResultEnum.TOKEN_ERROR:
        errorMsg = msgText || t('sys.api.timeoutMessage');
        const userStore = useUserStoreWithOut();
        userStore.setToken(undefined);
        userStore.logout(true);
        break;
      default:
        errorMsg = msgText || t('sys.api.apiRequestFailed');
    }
  ...
  requestInterceptors: (config, options) => {
    // 请求之前处理config
    (config as Recordable).headers['youyi-origin'] = 'pc';
