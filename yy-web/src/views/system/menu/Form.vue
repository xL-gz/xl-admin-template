    ...
    <BasicForm @register="registerForm">
      <template #icon="{ model, field }">
        <a-row type="flex">
          ...
          <a-form-item-rest>
            <youyi-color-picker v-model:value="propertyJson.iconBackgroundColor" size="small" :predefine="predefineList" name="iconBackground" />
          </a-form-item-rest>
        </a-row>
      </template>
      <template #selfUrl>
      ...
      <template #navigationIcon="{ model, field }">
        <a-form-item-rest>
          <div class="flex">
            <YouyiUploadImgSingle v-model:value="model[field]" tipText="文字图标" subTipText="210 × 60 px" />
            <div class="ml-15px">
              <YouyiUploadImgSingle v-model:value="dataForm.workLogoIcon" tipText="Logo图标" subTipText="60 × 60 px" />
            </div>
          </div>
        </a-form-item-rest>
      </template>
...
<script lang="ts" setup>
  ...
  function getSelfUrl(enCode) {
    ...
    state.selfUrl = `${window.location.origin}/YOUYI_APP_${enCode}`;
  }
