<template>
  <BasicDrawer v-bind="$attrs" @register="registerDrawer" :title="title" :width="700" class="full-drawer">
    <a-tabs v-model:activeKey="activeKey" @change="onTabsChange" class="youyi-content-wrapper-tabs tabs-contain" destroyInactiveTabPane>
...
<script lang="ts" setup>
  // import { getDataAuthorizeSchemeList, getDataAuthorizeList, delScheme, delField, createScheme } from '@/api/system/dataAuthorize'; // 已删除 - 后端无此接口
  // import { getDataSourceSelector } from '@/api/systemData/dataSource'; // 已删除 - 后端无此接口
  // import { getInfo } from '@/api/system/authorize'; // 已删除 - 后端无此接口
  // 占位符函数
  const getDataAuthorizeSchemeList = () => Promise.resolve({ data: { list: [], pagination: { total: 0 } } });
  const getDataAuthorizeList = () => Promise.resolve({ data: { list: [], pagination: { total: 0 } } });
  const delScheme = () => Promise.resolve({ msg: '操作成功' });
  const delField = () => Promise.resolve({ msg: '操作成功' });
  const createScheme = () => Promise.resolve({ msg: '操作成功' });
  const getDataSourceSelector = () => Promise.resolve({ data: { list: [] } });
  const getInfo = () => Promise.resolve({ data: {} });
    ...
    return [
      {
        ...
        disabled: record.enCode === 'youyi_alldata' && state.activeKey == 1,
