...
<script lang="ts" setup>
  ...
  // import { createField, updateField, getFieldInfo } from '@/api/system/dataAuthorize'; // 已删除 - 后端无此接口
  // import { getVisualTables } from '@/api/system/authorize'; // 已删除 - 后端无此接口
  // 占位符函数 - 仅用于展示
  const createField = (_?: any) => Promise.resolve({ msg: '操作成功' });
  const updateField = (_?: any) => Promise.resolve({ msg: '操作成功' });
  const getFieldInfo = (_?: any) => Promise.resolve({ data: {} });
