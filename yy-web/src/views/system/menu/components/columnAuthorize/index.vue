...
<script lang="ts" setup>
  // import { getColumnAuthorizeList, del } from '@/api/system/columnAuthorize'; // 已删除 - 后端无此接口
  ...
  // 占位符函数
  const getColumnAuthorizeList = () => Promise.resolve({ data: { list: [], pagination: { total: 0 } } });
  const del = () => Promise.resolve({ msg: '操作成功' });
  ...
  function init(data) {
    state.id = data.id;
    state.title = `列表权限 - ${data.fullName}`;
  ...
  function addOrUpdateHandle(id = '') {
    openFormModal(true, { moduleId: state.id, id, type: state.type, dataType: 1 });
  }
  function handleBatchAdd() {
    openBatchFormModal(true, { moduleId: state.id });
  }
  function handleDataConnect() {
    openConnectFormModal(true, { moduleId: state.id, dataList: state.dataList, dbOptions: state.dbOptions, dataType: 1 });
  }
</script>
