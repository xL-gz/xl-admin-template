...
<script lang="ts" setup>
  ...
  // import { create, update, getInfo } from '@/api/system/columnAuthorize'; // 已删除 - 后端无此接口
  // import { getVisualTables } from '@/api/system/authorize'; // 已删除 - 后端无此接口
  // 占位符函数 - 仅用于展示
  const create = (_?: any) => Promise.resolve({ msg: '操作成功' });
  const update = (_?: any) => Promise.resolve({ msg: '操作成功' });
  const getInfo = (_?: any) => Promise.resolve({ data: {} });
  const getVisualTables = (_?: any) => Promise.resolve({ data: [] });
  ...
  function init(data) {
    ...
    changeLoading(true);
    getVisualTables(state.moduleId, 1)
      .then(res => {
        let data: any[] = [];
