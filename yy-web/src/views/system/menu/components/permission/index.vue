<template>
  <BasicPopup v-bind="$attrs" @register="registerPopup" :title="title" class="full-popup">
    ...
    <div class="no-data" v-else>
      <youyi-empty :description="description" />
    </div>
  </BasicPopup>
</template>
<script lang="ts" setup>
  // import { getMenuPermissionGroup, getMenuPermissionInfo } from '@/api/system/menu'; // 已删除 - 后端无此接口
  // import { getPermissionGroup, getPermissionInfo } from '@/api/permission/permissionGroup'; // 已删除 - 后端无此接口
  // 占位符函数 - 仅用于展示
  const getMenuPermissionGroup = (_?: any) => Promise.resolve({ data: [] });
  const getMenuPermissionInfo = (_?: any) => Promise.resolve({ data: [] });
  const getPermissionGroup = (_?: any) => Promise.resolve({ data: [] });
  const getPermissionInfo = (_?: any) => Promise.resolve({ data: [] });
