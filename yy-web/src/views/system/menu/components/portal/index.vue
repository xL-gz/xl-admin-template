<template>
  <BasicPopup v-bind="$attrs" @register="registerPopup" :title="title" class="full-popup" destroy-on-close>
    ...
    <a-tabs v-model:activeKey="listQuery.platform" class="youyi-content-wrapper-tabs" destroyInactiveTabPane>
...
<script lang="ts" setup>
  ...
  // import { getPortalManageList, delPortalManage } from '@/api/onlineDev/portal'; // 已删除 - 后端无此接口
  // 占位符函数
  const getPortalManageList = () => Promise.resolve({ data: { list: [], pagination: { total: 0 } } });
  const delPortalManage = () => Promise.resolve({ msg: '操作成功' });
