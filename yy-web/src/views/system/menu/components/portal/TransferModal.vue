...
<script lang="ts" setup>
  ...
  // import { getModelData, setModelData } from '@/api/permission/authorize'; // 已删除 - 后端无此接口
  // import { getPermissionSelector } from '@/api/permission/permissionGroup'; // 已删除 - 后端无此接口
  // 占位符函数
  const getModelData = () => Promise.resolve({ data: { ids: [] } });
  const setModelData = () => Promise.resolve({ msg: '操作成功' });
  const getPermissionSelector = () => Promise.resolve({ data: { list: [] } });
