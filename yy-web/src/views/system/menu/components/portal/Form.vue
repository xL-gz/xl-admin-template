...
<script lang="ts" setup>
  ...
  // import { getPortalManageInfo, createPortalManage, updatePortalManage } from '@/api/onlineDev/portal'; // 已删除 - 后端无此接口
  // 占位符函数
  const getPortalManageInfo = () => Promise.resolve({ data: {} });
  const createPortalManage = () => Promise.resolve({ msg: '操作成功' });
  const updatePortalManage = () => Promise.resolve({ msg: '操作成功' });
