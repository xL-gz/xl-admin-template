  ...
  <BasicModal v-bind="$attrs" @register="registerModal" :width="800" :title="t('common.detailText')" :min-height="300" :footer="null" class="notice-modal">
    <div class="notice-wrapper">
      ...
      <div class="file-list" v-if="state.dataForm.files.length">
        <YouyiUploadFile v-model:value="state.dataForm.files" disabled detailed />
      </div>
    </div>
  </BasicModal>
</template>
<script lang="ts" setup>
  ...
  // import { getInfo, readInfo } from '@/api/system/message'; // 已删除 - 后端无此接口
  // 占位符函数
  const getInfo = () => Promise.resolve({ data: { title: '', releaseTime: '', releaseUser: '', bodyText: '', files: '[]' } });
  const readInfo = () => Promise.resolve({ data: { title: '', releaseTime: '', releaseUser: '', bodyText: '', files: '[]' } });
