...
<script lang="ts" setup>
  import { getPortalAuthorizeValues, updatePortalAuthorizeList, getFlowAuthorizeValues, updateFlowAuthorizeList } from '@/api/permission/authorize';
  ...
  const { title, portalAuthorizeTree, loading, btnLoading, type } = toRefs(state);

  function init(data) {
    state.objectId = data.id;
    state.btnLoading = false;
    state.title = data.fullName;
    state.type = data.type || 'portal';
    getAuthorizeList();
  }
  function getAuthorizeList() {
    ...
    const method = state.type == 'portal' ? getPortalAuthorizeValues : getFlowAuthorizeValues;
    method(state.objectId).then(res => {
  ...
  function handleSubmit() {
    ...
    const method = state.type == 'portal' ? updatePortalAuthorizeList : updateFlowAuthorizeList;
    method(state.objectId, { ids: state.portalIds })
      .then(res => {
