<template>
  <BasicDrawer v-bind="$attrs" @register="registerDrawer" title="内部聊天" width="340px" class="full-drawer chat-drawer" :closeFunc="handleClose">
    <a-tabs v-model:activeKey="activeKey" :tabBarGutter="10" class="average-tabs h-full" @change="onTabsChange" size="small">
      <a-tab-pane key="reply" tab="消息">
        <ScrollContainer>
          ...
          <youyi-empty v-if="!replyList.length" />
        </ScrollContainer>
      </a-tab-pane>
      <a-tab-pane key="contacts" tab="联系人">
        ...
        <ScrollContainer ref="infiniteBody" class="user-list">
          ...
          <youyi-empty v-if="!userList.length" />
        </ScrollContainer>
      </a-tab-pane>
    </a-tabs>
    <Im ref="imRef" />
...
<script lang="ts" setup>
  // import { getImUser } from '@/api/permission/user'; // 已删除 - 后端无此接口
  // import { getIMReply, deleteChatRecord, removeChatRecord } from '@/api/system/message'; // 已删除 - 后端无此接口
  // 占位符函数
  const getImUser = () => Promise.resolve({ data: { list: [] } });
  const getIMReply = () => Promise.resolve({ data: { list: [] } });
  const deleteChatRecord = () => Promise.resolve({ msg: '操作成功' });
  const removeChatRecord = () => Promise.resolve({ msg: '操作成功' });
  ...
  import { toDateText } from '@/utils/youyi';
