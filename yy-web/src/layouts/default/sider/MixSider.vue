<template>
  ...
  <div
    ...
    :style="getWrapStyle"
    :class="[prefixCls, getMenuTheme, { open: openMenu, mini: getCollapsed }]"
    ...
    <ScrollContainer>
      <ul :class="`${prefixCls}-module`">
        <li
          :class="[`${prefixCls}-module__item `, { [`${prefixCls}-module__item--active`]: item.path === activePath }]"
          v-bind="getItemEvents(item)"
          v-for="item in menuModules"
          :key="item.path">
          <SimpleMenuTag :item="item" collapseParent dot />
          <span :class="`${prefixCls}-module__icon ${item.icon}`"></span>
          <p :class="`${prefixCls}-module__name`">
            {{ t('routes.' + item.enCode, item.fullName) }}
          </p>
        </li>
      </ul>
    </ScrollContainer>
    ...
    <div :class="`${prefixCls}-menu-list`" ref="sideRef" :style="getMenuStyle">
      <div v-show="openMenu && false" :class="[`${prefixCls}-menu-list__title`, { show: openMenu }]">
    ...
    setup() {
      ...
      const getMenuStyle = computed((): CSSProperties => {
        return {
          ...
          left: `${unref(getMixSideWidth) + 10}px`,
        };
      });
      ...
      const getDomStyle = computed((): CSSProperties => {
        ...
        const width = `${unref(getMixSideWidth) + fixedWidth + 10}px`;
...
<style lang="less">
  ...
  html[data-theme='dark'] {
    .@{prefix-cls} {
      ...
      & > .ant-layout-sider-children > .scroll-container {
        background: #212121 !important;
      }
    }
  }
  .@{prefix-cls} {
    ...
    height: calc(100% - 10px - @header-height);
    overflow: hidden;
