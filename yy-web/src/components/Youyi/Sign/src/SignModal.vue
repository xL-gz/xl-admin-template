<template>
  <a-modal
    ...
    :title="t('component.youyi.sign.signPlaceholder')"
    class="youyi-sign-modal"
    centered
    :maskClosable="false"
    :keyboard="false"
    :width="600">
    ...
    <div class="sign-main">
      ...
      <div class="tip" v-show="!hasDrew">{{ t('component.youyi.sign.operateTip') }}</div>
    </div>
  </a-modal>
</template>
<script lang="ts" setup>
  // import { createSign } from '@/api/permission/userSetting'; // 已删除 - 后端无此接口
  // 占位符函数
  const createSign = () => Promise.resolve({ msg: '操作成功' });
  ...
  function handleSubmit() {
    (getESign() as unknown as Recordable)
      ...
      .catch(() => {
        createMessage.warning(t('component.youyi.sign.signPlaceholder'));
      });
  }
</script>
<style lang="less">
