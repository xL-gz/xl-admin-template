<template>
  <a-modal v-model:open="visible" title="选择签章" class="youyi-signature-modal" centered :maskClosable="false" :keyboard="false" :width="600">
    <template #closeIcon>
    ...
    <div class="sign-main">
      <a-form :colon="false" :labelCol="{ style: { width: '80px' } }" :model="dataForm" ref="formElRef">
        <a-form-item label="电子签章" name="innerValue">
          <youyi-select v-model:value="dataForm.innerValue" :options="options" showSearch allowClear />
        </a-form-item>
      </a-form>
    </div>
  </a-modal>
...
<script lang="ts" setup>
  // import { getListByIds } from '@/api/system/signature'; // 已删除 - 后端无此接口
  // 占位符函数
  const getListByIds = () => Promise.resolve({ data: { list: [] } });
