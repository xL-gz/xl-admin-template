...
<script lang="ts" setup>
  // import { getDataInterfaceDataSelect, getDataInterfaceDataInfoByIds } from '@/api/systemData/dataInterface'; // 已删除 - 后端无此接口
  // 占位符函数
  const getDataInterfaceDataSelect = () => Promise.resolve({ data: { list: [], pagination: { total: 0 } } });
  const getDataInterfaceDataInfoByIds = () => Promise.resolve({ data: [] });
  ...
  defineOptions({ name: 'YouyiPopupSelect', inheritAttrs: false });
  ...
  function setValue(value) {
    ...
    getDataInterfaceDataInfoByIds(props.interfaceId, query).then(res => {
      ...
      emitter.emit('setRelationData', { youyiRelationField: props.field, ...data });
    });
  }
