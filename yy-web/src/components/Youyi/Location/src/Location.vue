<template>
  <div :class="`${prefixCls}`">
    <a-button preIcon="icon-ym icon-ym-daka" :disabled="disabled" @click="handleLocation()" v-if="!detailed">
      {{ innerValue.fullAddress ? t('component.youyi.location.relocation') : t('component.youyi.location.location') }}
    </a-button>
  ...
  <a-modal
    ...
    :title="t('component.youyi.location.modalTitle')"
    :width="600"
    ...
    <div class="location-contain">
      <a-input-search v-model:value="keyword" :placeholder="t('component.youyi.location.searchPlaceholder')" allowClear @search="onSearch" />
      ...
      <ScrollContainer ref="infiniteBody" class="around-contain" v-loading="loading && listQuery.currentPage == 1">
        ...
        <!-- 请选择允许微调范围内的地点 -->
        <youyi-empty class="!mt-100px" v-if="!list.length && !loading" />
      </ScrollContainer>
    </div>
  </a-modal>
</template>
...
<script lang="ts" setup>
  ...
  defineOptions({ name: 'YouyiLocation', inheritAttrs: false });
  const defaultValue = {
    lat: '',
    lng: '',
    name: '',
