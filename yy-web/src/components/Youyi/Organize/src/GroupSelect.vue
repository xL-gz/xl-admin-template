...
<script lang="ts" setup>
  // import { getGroupByCondition } from '@/api/permission/group'; // 已删除 - 后端无此接口
  // 占位符函数
  const getGroupByCondition = () => Promise.resolve({ data: { list: [] } });
  ...
  defineOptions({ name: 'YouyiGroupSelect', inheritAttrs: false });
  ...
  function handleSelect(keys) {
    ...
    if (data?.disabled || data?.type !== 'group') return;
  ...
  async function initData() {
    loading.value = true;
    allList.value = await organizeStore.getGroupList();
    if (props.selectType === 'all') {
      treeData.value = await organizeStore.getGroupTree();
      loading.value = false;
    } else {
