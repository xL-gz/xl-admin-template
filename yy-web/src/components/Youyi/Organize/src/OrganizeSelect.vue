...
<script lang="ts" setup>
  // import { getOrganizeSelectorByAuth, getDepartmentSelectorByAuth, getOrgByOrganizeCondition } from '@/api/permission/organize'; // 已删除 - 后端无此接口
  // 占位符函数
  const getOrganizeSelectorByAuth = () => Promise.resolve({ data: { list: [] } });
  const getDepartmentSelectorByAuth = () => Promise.resolve({ data: { list: [] } });
