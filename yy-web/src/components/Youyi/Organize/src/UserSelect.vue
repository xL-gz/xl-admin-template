  ...
  <a-modal v-model:open="visible" :title="modalTitle" :width="800" class="transfer-modal" @ok="handleSubmit" centered :maskClosable="false" :keyboard="false">
    ...
    <div class="transfer__body">
      ...
      <div class="transfer-pane right-pane">
        <div class="transfer-pane__tool">
          <span>{{ t('component.youyi.common.selected') }}</span>
          <span class="remove-all-btn" @click="removeAll">{{ t('component.youyi.common.clearAll') }}</span>
        </div>
...
<script lang="ts" setup>
  // import { getImUserSelector, getUserInfoList, getSubordinates, getOrganization, getSelectedUserList } from '@/api/permission/user'; // 已删除 - 后端无此接口
  // 占位符函数
  const getImUserSelector = () => Promise.resolve({ data: { list: [] } });
  const getUserInfoList = () => Promise.resolve({ data: { list: [] } });
  const getSubordinates = () => Promise.resolve({ data: [] });
  const getOrganization = () => Promise.resolve({ data: [] });
  const getSelectedUserList = () => Promise.resolve({ data: { list: [] } });
  ...
  defineOptions({ name: 'YouyiUserSelect', inheritAttrs: false });
