<template>
  <a-popover
    ...
    trigger="click"
    class="youyi-color-picker"
    :class="[colorSize ? 'youyi-color-picker--' + colorSize : '']"
    v-bind="$attrs"
    v-if="!colorDisabled">
    <template #content>
      ...
      <div class="ant-color-dropdown__btns">
        ...
        <div class="ant-dropdown__btns">
          <a-button type="link" size="small" class="ant-cancel-button" @click="clear" v-if="allowClear">{{ t('common.cleanText') }}</a-button>
          <a-button size="small" @click="confirmValue">{{ t('common.okText') }}</a-button>
        </div>
      </div>
    </template>
    <div class="ant-color-picker__trigger" :style="{ borderRadius: sysBorderRadius }" @click="handleTrigger">
  ...
  <div
    class="youyi-color-picker ant-color-picker__trigger is-disabled"
    :class="[colorSize ? 'youyi-color-picker--' + colorSize : '']"
    :style="{ borderRadius: sysBorderRadius }"
    v-if="colorDisabled">
    <span class="ant-color-picker__color" :class="{ 'is-alpha': showAlpha }">
      <span class="ant-color-picker__color-inner" :style="{ backgroundColor: displayedColor }">
        <DownOutlined v-show="value || showPanelColor" style="color: #909399" />
...
<script lang="ts">
  ...
  export default defineComponent({
    ...
    setup(props, { emit }) {
      ...
      const sysBorderRadius = (themeToken.value.borderRadius || 0) + 'px';
...
<style lang="less" scoped>
  ...
  // large 大小
  .youyi-color-picker--large {
    width: 40px;
    height: 40px;
  }

  //small 大小
  .youyi-color-picker--small {
    width: 32px;
    height: 32px;
  }
</style>
