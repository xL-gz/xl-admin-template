...
<script lang="ts" setup>
  ...
  function beforeUpload(file) {
    ...
    if (isTopLimit) {
      createMessage.error({ content: t('component.upload.fileMaxNumber', [props.limit]), key: messageKey });
      return false;
    }
    ...
    if (!isRightSize) {
      createMessage.error({ content: t('component.upload.fileMaxSize', { size: props.fileSize, unit: props.sizeUnit }), key: messageKey });
      return isRightSize;
    }
    const isAccept = checkAccept(file);
    if (!isAccept) {
      createMessage.error({ content: t('component.upload.fileTypeCheck', [unref(getAcceptText)]), key: messageKey });
      return isAccept;
    }
  ...
  function computeMD5(file) {
    ...
    fileReader.onerror = function () {
      createMessage.error(t('component.upload.fileTypeCheck', [file.name]));
      file.cancel();
    };
