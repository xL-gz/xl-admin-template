<template>
  <div class="common-container">
    ...
    <!-- Detail组件引用已注释，因为对应的视图文件不存在 -->
    <!-- <Detail ref="detailRef" /> -->
  </div>
</template>

<script lang="ts" setup>
  // import { getFieldDataSelect, getDataChange, getConfigData } from '@/api/onlineDev/visualDev'; // 已删除 - 后端无此接口
  ...
  defineOptions({ name: 'YouyiRelationForm', inheritAttrs: false });
  ...
  function setValue(value) {
    ...
    if (!props.multiple) {
      ...
      //   emitter.emit('setRelationData', { youyiRelationField: props.field, ...data, id: res.data.id });
      // });
    }
  }
  function setNullValue() {
    ...
    generatorStore.setRelationData(relationData);
    emitter.emit('setRelationData', { youyiRelationField: props.field });
  }
  function onChange() {
    options.value = [];
    emit('update:value', '');
  ...
  function initData() {
    ...
    // getFieldDataSelect 已删除 - 后端无此接口
    // 以下功能需要后端支持或使用替代方案
    Promise.resolve({ data: { list: [], pagination: { total: 0 } } })
      .then(res => {
