<template>
  ...
  <a-modal
    ...
    :title="t('component.youyi.areaSelect.modalTitle')"
    :width="800"
    ...
    <div class="transfer__body">
      <div class="transfer-pane">
        <div class="transfer-pane__tool">
          <span>{{ t('component.youyi.common.allData') }}</span>
        </div>
...
<script lang="ts" setup>
  // import { getAreaSelector, getAreaByIds } from '@/api/system/area'; // 已删除 - 后端无此接口
  // 占位符函数
  const getAreaSelector = () => Promise.resolve({ data: { list: [] } });
  const getAreaByIds = () => Promise.resolve({ data: [] });
  ...
  defineOptions({ name: 'YouyiAreaSelect', inheritAttrs: false });
