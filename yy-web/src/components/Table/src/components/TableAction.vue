<template>
  <a-space :class="[prefixCls, getAlign]" @click="onCellClick">
    ...
    <Dropdown :trigger="['hover']" :dropMenuList="getDropdownList" :popconfirm="hasPopConfirm" v-if="dropDownActions && getDropdownList.length > 0">
      <slot name="more"></slot>
...
<script lang="ts">
  ...
  export default defineComponent({
    ...
    setup(props) {
        ...
        return list.map((action, index) => {
          ...
          return {
            ...
            popConfirm: popConfirm ? {
              ...popConfirm,
              confirm: popConfirm.confirm,
              cancel: popConfirm.cancel,
            } : undefined,
            onClick: popConfirm ? undefined : action.onClick,
            text: label,
      ...
      const hasPopConfirm = computed(() => {
        return getDropdownList.value.some(item => item.popConfirm);
      });
      ...
      return { t, prefixCls, getActions, getDropdownList, getAlign, onCellClick, getTooltip, hasPopConfirm };
    },
  });
</script>
<style lang="less">
