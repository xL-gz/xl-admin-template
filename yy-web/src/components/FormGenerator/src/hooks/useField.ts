...
export function useField(activeData) {
  ...
  function initFieldData() {
    const youyiKey = activeData.__config__.youyiKey;
    const list = ['popupTableSelect', 'popupSelect', 'autoComplete'];
    if (list.includes(youyiKey)) {
    ...
    const url = list.includes(youyiKey) ? activeData.interfaceId : activeData.__config__.propsUrl;
