<template>
  <a-form-item label="弹窗标题" v-if="youyiKey === 'popupSelect'">
    <a-input v-model:value="activeData.popupTitle" placeholder="请输入" />
  </a-form-item>
  <a-form-item label="弹窗类型" v-if="youyiKey === 'popupSelect' && showType === 'pc'">
    <youyi-select v-model:value="activeData.popupType" placeholder="请选择" :options="popupTypeOptions" />
  </a-form-item>
  <a-form-item label="弹窗宽度" v-if="youyiKey === 'popupSelect' && showType === 'pc'">
  ...
  <a-form-item v-if="youyiKey === 'popupSelect'">
    <template #label>填充字段<BasicHelp text="选择数据后，将远端数据的字段值填充到当前表单字段，不支持代码生成。" /></template>
  ...
  <a-form-item label="分页条数" v-if="activeData.hasPage">
    <youyi-radio v-model:value="activeData.pageSize" :options="pageSizeOptions" optionType="button" button-style="solid" class="right-radio" />
  </a-form-item>
  <a-divider></a-divider>
  <a-form-item label="能否清空">
    <a-switch v-model:checked="activeData.clearable" />
  </a-form-item>
  <a-form-item label="能否多选" v-if="youyiKey === 'popupTableSelect'">
