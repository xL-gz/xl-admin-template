<template>
  <a-form-item label="风格类型" v-if="showType === 'pc'">
    <youyi-radio v-model:value="activeData.type" :options="typeOptions" optionType="button" button-style="solid" class="right-radio" />
  </a-form-item>
  <a-form-item label="选项卡位置" v-if="showType === 'pc'">
    <youyi-radio v-model:value="activeData.tabPosition" :options="positionOptions" optionType="button" button-style="solid" class="right-radio" />
  </a-form-item>
  ...
  <div class="options-list">
    <draggable v-model="activeData.__config__.children" :animation="300" group="selectItem" handle=".option-drag" itemKey="uuid">
      <template #item="{ element, index }">
        <div class="select-item">
          ...
          <youyi-i18n-input v-model:value="element.title" v-model:i18n="element.titleI18nCode" placeholder="标签名称" />
    ...
    <div class="add-btn">
      <a-button type="link" preIcon="icon-ym icon-ym-btn-add" @click="addItem" class="!px-0">添加标签页</a-button>
    </div>
  </div>
</template>
