<template>
  <a-form-item label="编码格式">
    <youyi-select v-model:value="activeData.format" :options="barcodeFormatOptions" />
  </a-form-item>
  <a-form-item label="条码颜色">
    <youyi-color-picker v-model:value="activeData.lineColor" size="small" />
  </a-form-item>
  ...
  <a-form-item label="默认值">
    <youyi-select v-model:value="activeData.dataType" :options="dataTypeOptions" />
  </a-form-item>
  <a-form-item label="固定值" v-if="activeData.dataType === 'static'">
    <youyi-input v-model:value="activeData.staticText" placeholder="请输入" allowClear @change="onTextChange" />
  </a-form-item>
