<template>
  ...
  <a-form-item label="前图标" v-show="showType === 'pc'">
    <youyi-icon-picker v-model:value="activeData.prefixIcon" placeholder="请选择" />
  </a-form-item>
  <a-form-item label="后图标" v-show="showType === 'pc'">
    <youyi-icon-picker v-model:value="activeData.suffixIcon" placeholder="请选择" />
  </a-form-item>
  ...
  <div v-for="(item, index) in activeData.__config__.regList" :key="index" class="reg-item">
    ...
    <a-form-item label="错误提示" style="margin-bottom: 0">
      <youyi-i18n-input v-model:value="item.message" v-model:i18n="item.messageI18nCode" placeholder="请输入" />
    </a-form-item>
  </div>
  <div class="mt-10px">
    <a-dropdown>
...
<script lang="ts" setup>
  ...
  const ruleList = [
    {
      pattern: '/^\\d+$/',
      message: '请输入正确的数字',
      messageI18nCode: 'sys.validate.number',
      label: '数字',
    },
    {
      pattern: '/^([1-9][\\d]*|0)(\\.[\\d]+)?$/',
      message: '请输入正确的金额',
      messageI18nCode: 'sys.validate.money',
      label: '金额',
    },
    {
      pattern: '/^0\\d{2,3}-?\\d{7,8}$/',
      message: '请输入正确的电话号码',
      messageI18nCode: 'sys.validate.telephone',
      label: '电话',
    },
    {
    ...
    },
    {
      ...
      message: '请输入正确的身份证号码',
      messageI18nCode: 'sys.validate.idCard',
      label: '身份证',
    },
  ];
  ...
  function addReg(row?) {
    const item = {
      ...
      messageI18nCode: row?.messageI18nCode ? row?.messageI18nCode : '',
    };
