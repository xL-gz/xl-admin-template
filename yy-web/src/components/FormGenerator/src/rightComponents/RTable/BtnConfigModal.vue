    ...
    <a-form
      ...
      <a-form-item label="按钮名称" name="label">
        <youyi-i18n-input
          ...
          v-model:i18n="dataForm.labelI18nCode"
          placeholder="请输入"
          :disabled="['copy', 'remove'].includes(dataForm.value)"
          :maxlength="10" />
      </a-form-item>
      ...
      <template v-if="!['copy', 'remove'].includes(dataForm.value)">
        ...
        <a-form-item label="按钮图标">
          <youyi-icon-picker v-model:value="dataForm.btnIcon" placeholder="请选择" />
        </a-form-item>
      </template>
      ...
      <template v-if="!['copy', 'remove', 'add', 'batchRemove'].includes(dataForm.value)">
        <youyi-group-title content="动作设置" :bordered="false" />
        <a-form-item label="弹窗标题">
          <a-input v-model:value="actionConfig.popupTitle" placeholder="请输入" />
        </a-form-item>
        <a-form-item label="弹窗类型">
          <youyi-radio v-model:value="actionConfig.popupType" :options="popupTypeOptions" optionType="button" button-style="solid" class="right-radio" />
        </a-form-item>
            ...
            <template #bodyCell="{ column, record }">
              ...
              <template v-if="column.key === 'relationField'">
                ...
                <template v-else-if="record.sourceType == 2">
                  <youyi-input-number v-if="['int', 'decimal'].includes(record.dataType)" v-model:value="record.relationField" placeholder="请输入" clearable />
                  <youyi-date-picker
        ...
        <a-form-item label="表单数据" v-else>
          <YouyiTreeSelect v-model:value="actionConfig.modelId" :options="treeData" placeholder="请选择" lastLevel allowClear @change="onModeIdChange" />
        </a-form-item>
        ...
        <a-table :data-source="actionConfig.columnOptions" :columns="getColumnOptionsColumns" size="small" :pagination="false">
          ...
          <template #bodyCell="{ column, record, index }">
            ...
            <template v-if="column.key === 'value'">
              ...
              <youyi-select v-else v-model:value="record.value" placeholder="请选择" :options="fieldOptions" showSearch @change="onChange($event, record)" />
            </template>
        ...
        <a-table :data-source="actionConfig.relationOptions" :columns="relationOptionsColumns" size="small" :pagination="false">
          <template #bodyCell="{ column, record, index }">
            <template v-if="column.key === 'field'">
              <youyi-select v-model:value="record.field" placeholder="请选择" :options="childList" showSearch class="!w-134px" />
            </template>
            <template v-if="column.key === 'type'">
              <youyi-select v-model:value="record.type" placeholder="请选择" :options="typeOptions" @change="onTypeChange(record)" />
            </template>
            <template v-if="column.key === 'value'">
              <template v-if="record.type == 1">
                <a-auto-complete
                  ...
                  @focus="onFocus(record.value)"
                  @search="debounceOnSearch(record.value)" />
                <youyi-select v-else v-model:value="record.value" placeholder="请选择" :options="fieldOptions" showSearch />
              </template>
...
<script lang="ts" setup>
  ...
  const formFieldsOptions = computed(() => {
    ...
    const loop = (data, parent?) => {
      ...
      if (data.__vModel__ && !noAllowList.includes(data.__config__.youyiKey))
        list.push({
          id: data.__vModel__,
  ...
  function init(data) {
    ...
    childList.value = cloneDeep(data.children)
      .filter(o => o.__vModel__ && !noAllowSelectList.includes(o.__config__.youyiKey))
      .map(o => ({
        id: o.__vModel__,
