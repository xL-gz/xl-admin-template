    ...
    setup(props, { attrs }) {
      ...
      const layouts = {
        colFormItem(element, index, parent) {
          ...
          if (config.youyiKey == 'location') element.useAutoLocation = false;
          const Item =
            props.showType === 'app' && !useCompList.includes(config.youyiKey) ? (
        ...
        rowFormItem(element, index, parent) {
          ...
          if (element.__config__.youyiKey === 'tableGrid') {
          ...
          if (config.youyiKey === 'row' || (props.showType === 'app' && config.youyiKey === 'card')) {
            return (
              <a-col
                span={props.showType === 'app' ? 24 : config.span}
                data-draggable={true}
                ...
                <a-row class={className + ' drawing-row-item-row'}>
                  <span class="component-name">{config.youyiKey === 'card' ? config.label : config.componentName}</span>
                  <a-col>
