...
export default defineComponent({
  ...
  setup(props, { emit, expose }) {
    ...
    function buildDataObject(confClone, dataObject, formData) {
      const youyiKey = confClone.__config__.youyiKey;
      ...
      if (['calculate', 'table', 'barcode', 'qrcode', 'popupSelect', 'popupTableSelect', 'autoComplete'].includes(youyiKey)) {
        dataObject['formData'] = formData;
      }
      if (['table'].includes(youyiKey)) {
        dataObject['relations'] = props.relations;
        dataObject['vModel'] = confClone.__vModel__;
        dataObject['ref'] = 'tableRef';
      }
      if (['relationForm', 'popupSelect'].includes(youyiKey)) {
        dataObject['field'] = confClone.__config__.tableName
          ? confClone.__vModel__ + '_youyiTable_' + confClone.__config__.tableName + (confClone.__config__.isSubTable ? '0' : '1')
    ...
    return () => {
      ...
      const youyiKey = upperFirst(props.conf.__config__.youyiKey) === 'Table' ? 'InputTable' : upperFirst(props.conf.__config__.youyiKey);
      const Comp = componentMap.get(youyiKey as ComponentType) as ReturnType<typeof defineComponent>;
      if (!Comp) return null;
      const realDataObject = getRealProps(dataObject, props.conf.__config__.youyiKey);
