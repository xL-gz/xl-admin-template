      ...
      <template #bodyCell="{ column, record, index }">
        ...
        <template v-else>
          ...
          <template v-if="column.key === 'dataType'">
            <youyi-select v-model:value="record.dataType" placeholder="请选择" :options="options" />
          </template>
...
<script lang="ts" setup>
  // import { getDataModelInfo as getInfo, addTableFields } from '@/api/systemData/dataModel'; // 已删除 - 后端无此接口
  // 占位符函数 - 仅用于展示
  const getInfo = (_?: any) => Promise.resolve({ data: {} });
  const addTableFields = (_?: any) => Promise.resolve({ msg: '操作成功' });
  import { ref, unref } from 'vue';
  import { BasicModal, useModalInner } from '@/components/Modal';
  import { useMessage } from '@/hooks/web/useMessage';
  import { getDataTypeText } from '@/utils/youyi';
