<script lang="tsx">
  ...
  import { getScriptFunc, onlineUtils, getDateTimeUnit, getTimeUnit, getParamList } from '@/utils/youyi';
    ...
    setup(props, { expose, emit }) {
      ...
      const layouts = {
        colFormItem(element) {
          ...
          if (['groupTitle', 'divider', 'link', 'text'].includes(config.youyiKey)) {
          ...
          if (config.youyiKey === 'button') {
            if (element.buttonTextI18nCode) element.buttonText = t(element.buttonTextI18nCode, element.buttonText);
          }
          if (config.youyiKey === 'alert') {
          ...
          const Item = (
            <render
              ...
              {...listeners}
              ref={config.youyiKey === 'table' ? element.__vModel__ : undefined}
              relations={config.youyiKey === 'table' ? state.relations : undefined}
            />
          );
          ...
          Object.keys(scheme.on).forEach(key => {
            ...
            listeners['on' + upperFirst(key)] = (...arg) => {
              if (key === 'change') {
                ...
                if (['popupSelect', 'relationForm'].includes(config.youyiKey)) setTransferFormData(data, config);
      ...
      function comSet(field, prop, value) {
        ...
        const loop = list => {
          for (let i = 0; i < list.length; i++) {
            ...
            if (item.__vModel__ && item.__vModel__ === prop) {
              switch (field) {
                ...
                case 'endTime':
                  ...
                  break;
                case 'options':
                  if (dyOptionsList.indexOf(item.__config__.youyiKey) > -1) item.options = value;
                  break;
                default:
            ...
            if (item.__config__ && item.__config__.youyiKey !== 'table' && item.__config__.children && Array.isArray(item.__config__.children)) {
      ...
      function initFormData(componentList) {
        ...
        componentList.forEach(cur => {
          ...
          if (cur.__config__.youyiKey == 'table') return;
      ...
      function initRelationForm(componentList) {
        componentList.forEach(cur => {
          const config = cur.__config__;
          if (config.youyiKey == 'relationFormAttr' || config.youyiKey == 'popupAttr') {
            const relationKey = cur.relationField.split('_youyiTable_')[0];
      ...
      function buildRules(componentList) {
        componentList.forEach(cur => {
          ...
          if (config.children && config.youyiKey !== 'table') buildRules(config.children);
        });
      }
      ...
      function buildOptions(componentList) {
        componentList.forEach(cur => {
          const config = cur.__config__;
          if (dyOptionsList.includes(config.youyiKey)) {
          ...
          if (config.children && config.youyiKey !== 'table') buildOptions(config.children);
        });
      }
      function buildRelations(componentList, relations) {
        componentList.forEach(cur => {
          ...
          if (config.youyiKey === 'userSelect' && ['dep', 'pos', 'role', 'group'].includes(cur.selectType)) {
