...
<script lang="ts" setup>
  // import { getDataInterfaceDataSelect } from '@/api/systemData/dataInterface'; // 已删除 - 后端无此接口
  // import { getFieldDataSelect } from '@/api/onlineDev/visualDev'; // 已删除 - 后端无此接口
  // 占位符函数 - 仅用于展示
  const getDataInterfaceDataSelect = (_?: any) => Promise.resolve({ data: { list: [], pagination: { total: 0 } } });
  const getFieldDataSelect = (_?: any) => Promise.resolve({ data: { list: [], pagination: { total: 0 } } });
  ...
  function handleSubmit() {
    ...
    for (let i = 0; i < unref(selectRow).length; i++) {
      ...
      for (let j = 0; j < props.config.relationOptions.length; j++) {
        ...
        item[row.field] = row.type === 1 ? e[!unref(getIsDynamic) ? row.value + '_youyiId' : row.value] : row.value;
      }
      checked.push(item);
    }
    emit('select', checked);
  ...
  function getParamList() {
    ...
    for (let i = 0; i < templateJson.length; i++) {
      ...
      if (e.youyiKey === 'createUser') e.defaultValue = unref(getUserInfo).userId;
      if (e.youyiKey === 'createTime') e.defaultValue = new Date().getTime();
      if (e.youyiKey === 'currOrganize') e.defaultValue = unref(getUserInfo).organizeId;
      if (e.youyiKey === 'currPosition' && unref(getUserInfo).positionIds?.length) e.defaultValue = unref(getUserInfo).positionIds[0].id;
    }
    return templateJson;
  }
